...
export default function SearchTranscripts() {
  ...
  useEffect(() => {
    const fetchInitial = async () => {
      setIsSearching(true);
      try {
        const response = await fetch('${import.meta.env.VITE_API_URL}api/search?q=&genre=All%20Categories&sentiment=All%20Sentiments&emotion=All%20Emotions&character=&page=1&limit=10');
        if (response.ok) {
  ...
  const fetchResults = async (pageNum) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/search?q=${queryStr}`);
